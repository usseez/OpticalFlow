...
def get_divisible_size(width, height, divisor=64):
    """
    PWC-Net에 맞는 크기로 조정 (divisor의 배수)
    """
...
def compute_flow(model, device, frame1, frame2):
    ...
    # PWC-Net을 위한 크기 조정 (64의 배수)
    model_w, model_h = get_divisible_size(w, h, divisor=64)
    
    # 리사이즈
    frame1_resized = cv2.resize(frame1, (model_w, model_h))
    frame2_resized = cv2.resize(frame2, (model_w, model_h))
    ...
    # print(f'img_pair: {img_pair.shape}')
